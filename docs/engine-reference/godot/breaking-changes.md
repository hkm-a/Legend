...
## 4.6 → 4.6.3 (May 2026 — POST-CUTOFF, HIGH RISK)

Godot 4.6.3 is a stable maintenance patch release on the 4.6 branch. Official
release summaries identify it as a maintenance/stable release, with 86 fixes
from 41 contributors. No new project-level migration break from 4.6 to 4.6.3
was identified in the search results; still verify any API-sensitive work against
stable docs and keep backups/version control when upgrading.

| Subsystem | Change | Details |
|-----------|--------|---------|
| Stability | Maintenance fixes | 4.6.3 is a patch release for the 4.6 branch, focused on stability and usability fixes. |
| Compatibility | Same 4.6 branch | Treat as compatible with 4.6 unless official release notes for a touched subsystem say otherwise. |
| Project policy | Pin exact patch | Use Godot 4.6.3 for this project so CI/export/editor behavior stays consistent. |
...
| C# | Automatic string extraction | Translation strings auto-extracted from C# code. Not relevant to this GDScript-primary project unless C# is added later. |
