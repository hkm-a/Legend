...
static var _encode_map: Dictionary = {}
static var _encode_map_ready := false
...
static func decode(bytes: PackedByteArray) -> String:
	...
	while index < bytes.size():
		...
		if first < 0x80:
			result += String.chr(first)
			index += 1
			continue
		if index + 1 >= bytes.size():
			result += String.chr(REPLACEMENT_CODEPOINT)
			break
		var second := int(bytes[index + 1])
		result += _decode_double_byte(first, second)
		index += 2
	return result
...
static func encode(text: String) -> PackedByteArray:
	...
	for index in range(text.length()):
		...
		elif _encode_map.has(codepoint):
			var packed := int(_encode_map[codepoint])
			bytes.append((packed >> 8) & 0xFF)
			bytes.append(packed & 0xFF)
		else:
			bytes.append(0x3F)
	return bytes

static func _decode_double_byte(first: int, second: int) -> String:
	if first < 0xA1 or first > 0xFE or second < 0xA1 or second > 0xFE:
		return String.chr(REPLACEMENT_CODEPOINT)
	var table_index := (first - 0xA1) * 94 + (second - 0xA1)
	var codepoint := GB2312_TABLE.unicode_at(table_index)
	if codepoint == REPLACEMENT_CODEPOINT:
		return String.chr(REPLACEMENT_CODEPOINT)
	return String.chr(codepoint)

static func _ensure_encode_map() -> void:
	if _encode_map_ready:
		return
	for table_index in range(GB2312_TABLE.length()):
		var codepoint := GB2312_TABLE.unicode_at(table_index)
		if codepoint == REPLACEMENT_CODEPOINT or _encode_map.has(codepoint):
			continue
		var first := 0xA1 + int(table_index / 94)
		var second := 0xA1 + int(table_index % 94)
		_encode_map[codepoint] = (first << 8) | second
	_encode_map_ready = true
